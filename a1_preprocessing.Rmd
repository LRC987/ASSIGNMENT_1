...
knitr::opts_knit$set(root.dir = '/Users/gloria/Desktop/Gloria_study/Methods_3/code/Assignment_1')
...
# Load the Data on utterance complexity (MLU)
LU_train <- read.csv("LU_train.csv")

# Load the Data on lexical complexity
...
#demo_train <- demo_train %>%
 # {colnames(.)[colnames(.) == "Child.ID"] <- "SUBJ"; .} %>%
  #{colnames(.)[colnames(.) == "Visit"] <- "VISIT"; .}
...
# Extract digits from "visit" column
LU_train$VISIT <- as.numeric(str_extract(LU_train$VISIT, "\\d"))
token_train$VISIT <- as.numeric(str_extract(token_train$VISIT, "\\d"))


# Now, all "visit" values are represented as digits
LU_train
token_train
...
# Custom function to remove last character, except when "." is the second character
remove_last_character_except_abr <- function(x) {
  ifelse(substr(x, 2, 2) == ".", x, substr(x, 1, nchar(x) - 1))
}

#apply the function to the datasets
LU_train <- LU_train %>%
  mutate(SUBJ = remove_last_character_except_abr(SUBJ))
...
merged_data <- dplyr::bind_rows(demo_train, LU_train, token_train)

#rename some columns
merged_data <- merged_data %>%
  rename(Visit = VISIT, Child.ID = SUBJ)

# Check for data loss and NAs
data_loss <- nrow(merged_data) - sum(complete.cases(merged_data))
nas <- sum(is.na(merged_data))

# Print information
cat("Data Loss: ", data_loss, " rows\n")
cat("NAs in Merged Data: ", nas, "\n")
...
### Write your code here
# Create a new dataset with rows where "visit" is equal to 1
visit1_data <- subset(merged_data, Visit == 1)
...
visit1_data <- merged_data %>%
  filter(Visit == 1) %>%
  select(Child.ID, ADOS, MullenRaw, ExpressiveLangRaw, Socialization) %>%
  rename(
    ADOS1 = ADOS,
...
# Merge the datasets using bind_cols
merged_data2 <- dplyr::bind_rows(merged_data, visit1_data)
merged_data2

# Merge the new dataset with the old dataset based on child.ID
#merged_data2 <- left_join(merged_data, visit1_data, by = "SUBJ")
```
...
# change 1 and 2 to F and M respectively
#ifelse() to conditionally change the values. If "gender" is equal to 1, it will be changed to "F," and if it's equal to 2, it will be changed to "M."
merged_data2 <- merged_data2 %>%
  mutate(Gender = ifelse(Gender == 1, "F", "M"))

#transform the 'Diagnosis' variable so that 'A's and 'B's are reverted back to ASD (Autism Spectrum Disorder) and TD (typically developing) respectively.
cleaned_data <- merged_data2%>%
  mutate(Diagnosis = case_when(
    Diagnosis == 'A' ~ 'ASD',
    Diagnosis == 'B' ~ 'TD',
    TRUE ~ Diagnosis  # Keep other values unchanged
  ))
...
```{r save_changes}
### Write your code here
#maybe rename some columns to something different
# Specify the file path and name where you want to save the CSV file
file_path <- "/Users/gloria/Desktop/Gloria_study/Methods_3/code/Assignment_1/cleaned_data.csv"
